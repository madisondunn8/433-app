...
#draft_scores_records %>% 
  ...
  #ggplot(aes(x = draft_score, y = win_pct_3_years_after)) + 
  #geom_point()

draft_scores_records %>% 
  #filter(best_pick <= 3) %>%
  ggplot(aes(x = draft_score, y = win_pct_change_3_years)) + 
  geom_point()+
  geom_smooth()+
  ggtitle("Win % Five Years After Current Draft vs Draft Score in Current Draft")

#draft_scores_records %>% 
  #filter(best_pick <= 30) %>%
  #ggplot(aes(x =  best_pick, y = win_pct_change_3_years)) + 
  #geom_point()
...
draft_scores_records %>% 
  ...
  geom_point()+
  geom_smooth()+
  ggtitle("Win % Five Years After Current Draft vs Best Pick in Current Draft")
```
